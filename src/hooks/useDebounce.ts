...
export function useDebounceState<S>(
  ...
  const [status, setStatus] = useState<DebounceValueStatus>(
    DebounceValueStatus.DONE
  );
...
interface UseDebounceConfigOptionsForPromise<TSuccess>
  extends BaseUseDebounceConfigOptions {
...
type UseDebounceConfigOptions<TReturn> = TReturn extends Promise<infer TResolve>
  ? UseDebounceConfigOptionsForPromise<TResolve>
  : BaseUseDebounceConfigOptions;
...
>(
  ...
  const [progressStatus, setProgressStatus] = useState<DebounceValueStatus>(
    DebounceValueStatus.DONE
  );
  ...
  const funcWithoutDeps = (...args: TParams) => {
    ...
    timer.current = setTimeout(() => {
      ...
      const promiseCallbacks = others as Omit<
        UseDebounceConfigOptions<Promise<any>>,
        "debounceTime"
      >;
      funcReturn
        .then((data) => {
